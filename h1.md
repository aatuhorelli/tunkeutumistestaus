...
Kytkin palomuurin päälle komennolla ``$ sudo ufw enable``, ja latasin WebGoatin ``$ wget https://github.com/WebGoat/WebGoat/releases/download/v2023.4/webgoat-2023.4.jar``, minkä jälkeen käynnistin sen komennolla ``$ java -Dfile.encoding=UTF-8 -Dwebgoat.port=8888 -Dwebwolf.port=9090 -jar webgoat-2023.4.jar``. Siirryin selaimella osoitteeseen localhost:8888/webgoat/. Palvelu oli päällä ja onnistuin luomaan tunnukset 'Register new user'-ominaisuuden kautta.
