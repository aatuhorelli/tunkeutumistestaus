...
Seurasin tehtävänantoon linkattua WebGoatin [asennusohjetta](https://terokarvinen.com/2020/install-webgoat-web-pentest-practice-target/). Avasin virtuaalikoneen terminaalin, päivitin paketinhallinnan listat ``$ sudo apt-get update`` ja latasin WebGoatin pyörittämiseen tarvittavat paketit ``$ sudo apt-get -y install openjdk-17-jre ufw wget bash-completion``. 

Kytkin palomuurin päälle komennolla ``$ sudo ufw enable``, ja latasin WebGoatin ``$ wget https://terokarvinen.com/2020/install-webgoat-web-pentest-practice-target/webgoat-server-8.0.0.M26.jar``, minkä jälkeen käynnistin sen komennolla ``$java -jar webgoat-server-8.0.0.M26.jar``. Siirryin selaimella osoitteeseen localhost:8080/webgoat/. Palvelu oli päällä ja onnistuin luomaan tunnukset 'Register new user'-ominaisuuden kautta.

![Add file: webgoat toimii](/img/h1/webgoat_paalla.png)
>Vasemmalla WebGoatin prosessi terminaalissa ja oikealla WebGoat sisäänkirjautumisen jälkeen

Lähde: https://terokarvinen.com/2020/install-webgoat-web-pentest-practice-target/. Luettu 6.4.2024. 

## C) WebGoat HTTPS Basics & Developer tools
